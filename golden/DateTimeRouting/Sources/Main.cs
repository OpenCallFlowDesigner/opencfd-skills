...
namespace DateTimeRouting
{
   public class Main : ScriptBase<Main>, ICallflow, ICallflowProcessor
   {
      ...
      private void InitializeVariables(string callID)
      {
         ...
         // User variables
         variableMap["callflow$.BusinessClosedQueue"] = new Variable(800);
            variableMap["callflow$.PromotionsQueue"] = new Variable(801);
            variableMap["callflow$.StandardQueue"] = new Variable(802);
            variableMap["RecordResult.NothingRecorded"] = new Variable(RecordComponent.RecordResults.NothingRecorded);
            variableMap["RecordResult.StopDigit"] = new Variable(RecordComponent.RecordResults.StopDigit);
            variableMap["RecordResult.Completed"] = new Variable(RecordComponent.RecordResults.Completed);
            variableMap["MenuResult.Timeout"] = new Variable(MenuComponent.MenuResults.Timeout);
      ...
      private void InitializeComponents(ICallflow callflow, ICall myCall, string logHeader)
      {
...

         {
            ...
            mainFlowComponentList.Add(dateTimeCondition);
            ...
            TransferComponent transferBusinessClosed = scope.CreateComponent<TransferComponent>("transferBusinessClosed");
            transferBusinessClosed.DestinationHandler = () => { return Convert.ToString(variableMap["callflow$.BusinessClosedQueue"].Value); };
            transferBusinessClosed.DelayMilliseconds = 500;
            dateTimeCondition.ContainerList[0].ComponentList.Add(transferBusinessClosed);
            dateTimeCondition.ConditionList.Add(() => { return Convert.ToBoolean(((DateTime.Now.DayOfWeek == DayOfWeek.Monday || DateTime.Now.DayOfWeek == DayOfWeek.Wednesday || DateTime.Now.DayOfWeek == DayOfWeek.Friday) && (DateTime.Now.Hour > 9 || DateTime.Now.Hour == 9 && DateTime.Now.Minute >= 0) && (DateTime.Now.Hour < 11 || DateTime.Now.Hour == 11 && DateTime.Now.Minute <= 59) || (DateTime.Now.DayOfWeek == DayOfWeek.Monday || DateTime.Now.DayOfWeek == DayOfWeek.Wednesday || DateTime.Now.DayOfWeek == DayOfWeek.Friday) && (DateTime.Now.Hour > 14 || DateTime.Now.Hour == 14 && DateTime.Now.Minute >= 0) && (DateTime.Now.Hour < 16 || DateTime.Now.Hour == 16 && DateTime.Now.Minute <= 59))); });
            dateTimeCondition.ContainerList.Add(scope.CreateComponent<SequenceContainerComponent>("dateTimeCondition_1"));
            TransferComponent transferPromotions = scope.CreateComponent<TransferComponent>("transferPromotions");
            transferPromotions.DestinationHandler = () => { return Convert.ToString(variableMap["callflow$.PromotionsQueue"].Value); };
            transferPromotions.DelayMilliseconds = 500;
            dateTimeCondition.ContainerList[1].ComponentList.Add(transferPromotions);
            dateTimeCondition.ConditionList.Add(() => { return Convert.ToBoolean((officeHoursManager.IsOfficeHours())); });
            dateTimeCondition.ContainerList.Add(scope.CreateComponent<SequenceContainerComponent>("dateTimeCondition_2"));
            TransferComponent transferStandard = scope.CreateComponent<TransferComponent>("transferStandard");
            transferStandard.DestinationHandler = () => { return Convert.ToString(variableMap["callflow$.StandardQueue"].Value); };
            transferStandard.DelayMilliseconds = 500;
            dateTimeCondition.ContainerList[2].ComponentList.Add(transferStandard);
            }
            {
            }
            {
      ...
      public override void Start()
      {
         ...
         string logHeader = $"DateTimeRouting - CallID {callID}";
         this.logFormatter = new LogFormatter(MyCall, logHeader, "Callflow");
         this.promptQueue = new PromptQueue(this, MyCall, "DateTimeRouting", logHeader);
